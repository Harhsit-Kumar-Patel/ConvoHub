    ...
    <Card>
      <CardHeader>
        <div className="flex justify-between items-start">
          ...
          <span className={`text-xs px-2 py-1 rounded-full ${submission.grade ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>
            {submission.grade ? `Graded: ${submission.grade}` : 'Not Graded'}
          </span>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {submission.files && submission.files.length > 0 && (
          <div>
            <label className="text-xs font-semibold text-muted-foreground">Submitted Files</label>
            <div className="mt-1 space-y-2">
              {submission.files.map((file) => (
                ...
                >
                  <Icons.notice className="w-4 h-4" />
                  {file.originalName}
                </a>
              ))}
            </div>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <div className="sm:col-span-1">
            <label htmlFor={`grade-${submission._id}`} className="text-sm font-medium">
              Grade
            </label>
          ...
          <div className="sm:col-span-2">
            <label htmlFor={`feedback-${submission._id}`} className="text-sm font-medium">
              Feedback
            </label>
...
export default function Grading() {
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true);
      try {
        const [asgmtRes, subsRes] = await Promise.all([api.get(`/assignments/${id}`), api.get(`/assignments/${id}/submissions`)]);
  ...
  const handleGradeUpdate = (updatedSubmission) => {
    setSubmissions((prev) => prev.map((sub) => (sub._id === updatedSubmission._id ? { ...sub, ...updatedSubmission } : sub)));
  };
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="space-y-4">
        {submissions.length > 0 ? (
          submissions.map((sub) => <SubmissionCard key={sub._id} submission={sub} assignmentId={id} onGradeSave={handleGradeUpdate} />)
        ) : (
          <Card>
